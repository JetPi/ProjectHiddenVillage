  ...
  return (
    <div className="mt-3 space-y-4">
      ...
      <div className="grid grid-cols-1 gap-2">
        <label className="text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)]" htmlFor="conditions-json">
          Conditions JSON
        </label>
        <textarea
          id="conditions-json"
          value={editorModel.draft.conditionsText}
          onChange={(event) => editorModel.setConditionsText(event.target.value)}
          className="min-h-[120px] w-full rounded-lg border border-[var(--border-subtle)] bg-[var(--surface)] px-3 py-2 font-mono text-xs text-[var(--text-primary)]"
        />
        {editorModel.errors.conditionsText ? (
          <p className="text-xs text-red-500">{editorModel.errors.conditionsText}</p>
        ) : null}
      </div>
